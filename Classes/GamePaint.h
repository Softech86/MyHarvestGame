...
public:
    ...
    LiveCode objAlpha(LiveCode object, float oldalpha, float newalpha, float timeSec, float delaySec = 0);
    void objRemove(LiveCode object, LiveCode parent);
    
    void objZOrder(LiveCode object, int ZOrder);
	void objZOrder(LiveCode object, int oldOrder, int newOrder, float timeSec, float delaySec);
